...
Future<List> fetchWpPostsEng() async {
  var numPage = 1;
  var perPage = 50;
  var convertDatatoJson;
  var isCacheExit = await APICacheManager().isAPICacheKeyExist("Home_API_Eng");
  ...
  if (!isCacheExit) {
    ...
    for (numPage = 2; numPage <= totalPage; numPage++) {
      var url = 'https://mtnewstoday.com/en/wp-json/wp/v2/posts?_embed&page=$numPage&per_page=$perPage';
      var response = await http.get(Uri.parse(url), headers: {'Content-Type': 'application/json'});
      // var url =
      //     'https://click4pdf.org/wp-json/wp/v2/posts?_embed&page=$numPage&per_page=$perPage';
    ...
    var cacheData = await APICacheManager().getCacheData("Home_API_Eng");
    // if (kDebugMode) {
    //   print("Cache hit");
    // }

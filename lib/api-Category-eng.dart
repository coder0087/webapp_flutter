...
Future<List> fetchWpPostsBusinessEng(int? id) async {
  var numPage = 1;
  var perPage = 50 ;
  var convertDatatoJson;
  var isCacheExit = await APICacheManager().isAPICacheKeyExist("allCategory_API_Eng$id");
  ...
  if (!isCacheExit) {
    ...
    if(totalPosts>perPage){
      for(numPage = 2 ; numPage<=totalPages; numPage++){
        var url = 'https://mtnewstoday.com/en/wp-json/wp/v2/posts?_embed&categories=$id&page=$numPage&per_page=$perPage';
        var response = await http.get(Uri.parse(url), headers: {'Content-Type': 'application/json'});
        // var url = 'https://click4pdf.org/wp-json/wp/v2/posts?_embed&categories=$id&page=$numPage&per_page=$perPage';
        // var response =
        // await http.get(Uri.parse(url), headers: {'Content-Type': 'application/json',
    ...
    var cacheData = await APICacheManager().getCacheData("allCategory_API_Eng$id");
    // if (kDebugMode) {
    //   print("Cache hit");
    // }

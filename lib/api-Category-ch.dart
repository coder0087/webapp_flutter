...
Future<List> fetchWpPostsBusinessCh(int? id) async {
  var numPage = 1;
  var perPage = 50 ;
  var convertDatatoJson;
  var isCacheExit = await APICacheManager().isAPICacheKeyExist("allCategory_API_Ch$id");

  if (!isCacheExit) {
    // var url = 'https://click4pdf.org/wp-json/wp/v2/posts?_embed&categories=$id&page=$numPage&per_page=$perPage';
    var url = 'https://mtnewstoday.com/ch/wp-json/wp/v2/posts?_embed&categories=$id&page=$numPage&per_page=$perPage';
    ...
    var cacheData = await APICacheManager().getCacheData("allCategory_API_Ch$id");
    // if (kDebugMode) {
    //   print("Cache hit");
    // }

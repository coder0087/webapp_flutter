...
import 'package:wordpress1/uiView/home-ch.dart';
...
class _HomeEngState extends State<HomeEng> {
  ...
  Future checkConnection() async {
    try {
      ...
      if (result.isEmpty && result[0].rawAddress.isEmpty) {
        setState(() {fetchAlbum = fetchWpPostsEng();});

      } else {
        setState(() {APICacheManager().deleteCache("Home_API_Eng");
        fetchAlbum = fetchWpPostsEng();});
  ...
  // var numm = List.filled(10, 0, growable: true);
          ...
          PopUpMen(
            menuList: [
              ...
              PopupMenuItem(
                ...
                onTap: () => Future(
                      () => Navigator.of(context).push(
                      MaterialPageRoute(builder: (_) => HomeCh())
                  ),
                ),
                            ...
                            ListTile(
                              ...
                              onTap: () {
                                Navigator.push(
                                    context,
                                    MaterialPageRoute(
                                        builder: (context) => CategoryHomeEng(
                                            id: wwCategory['id'],
                                            titleCategory:
                                                wwCategory['name'])));
                              },
                  ...
                  if (snapshot.hasError) {
                    ...
                    return ListView.builder(
                      ...
                      itemBuilder: (context, int index) {
                        // print("index real => $index ");
                        // print("Type Index => ${index.runtimeType}");
                        ...
                        // for(int hhh=0; hhh < numm.length; hhh++){
                        //   if(index!=numm[hhh]){
                        //     Map wwpost = snapshot.data[index];
                        //     return Card(
                        //       child: Padding(
                        ...
                        //                 // href: wwpost['_links']['wp:featuredmedia'][0]
                        //                 // ['href'],
                        //                 href: wwpost['_embedded']['wp:featuredmedia'][0]['source_url'],
                        //                 title: wwpost['title']['rendered']
                        //                     .replaceAll("&#8217;", "'"),
                        //                 desc: (parse((wwpost['excerpt']['rendered'])
                        //                     .toString())

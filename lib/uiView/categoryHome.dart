...
class BusinessHome extends StatefulWidget {
  ...
  final int? id;
  BusinessHome({this.id,this.titleCategory});
  @override
  ...
  Future checkConnection() async {
    try {
      ...
      if (result.isEmpty && result[0].rawAddress.isEmpty) {
        setState(() {fetchAlbum = fetchWpPostsBusiness(widget.id!);});

      } else {
        setState(() {APICacheManager().deleteCache("allCategory_API${widget.id!}");
        fetchAlbum = fetchWpPostsBusiness(widget.id!);});
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
        ...
        body: RefreshIndicator(
          ...
          child: FutureBuilder(
              future: fetchWpPostsBusiness(widget.id!),

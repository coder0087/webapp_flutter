...
class CategoryHomeCh extends StatefulWidget {
  final String? titleCategory;
  final int? id;
  CategoryHomeCh({this.id,this.titleCategory});
  @override
  State<CategoryHomeCh> createState() => _CategoryHomeChState();
}

class _CategoryHomeChState extends State<CategoryHomeCh> {
  ...
  Future checkConnection() async {
    try {
      ...
      if (result.isEmpty && result[0].rawAddress.isEmpty) {
        setState(() {fetchAlbum = fetchWpPostsBusinessCh(widget.id!);});

      } else {
        setState(() {APICacheManager().deleteCache("allCategory_API_Ch${widget.id!}");
        fetchAlbum = fetchWpPostsBusinessCh(widget.id!);});
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
        ...
        body: RefreshIndicator(
          ...
          child: FutureBuilder(
              future: fetchWpPostsBusinessCh(widget.id!),
